 ...
 */
function writeTextFiles(files, overwrite, verbose = false) {
    ...
    try {
      if (!overwrite && process.stdin.isTTY && fs.existsSync(filePath)) {
        ...
        if (response === "y" || response === "a") {
          ...
          writeTextFile(filePath, content, true, verbose)
        }
        else if (response === "n") continue
        else if (response === "c") break
      }
      else {
        writeTextFile(filePath, content, overwrite, verbose)
      }
    }
    catch (err) {
      const colon = err.message.indexOf(":")
 ...
 * @private
 */
function writeTextFile(filePath, content, overwrite, verbose) {
