 ...
 */
function printError(err, displayParseError = false) {
  ...
  // so by default they're handled separately via printParseError()
  if (err.token && !isErrorObj) {
    if (displayParseError) {
      console.error(chalk.red(`Parse error on line ${err.token.lineNum}: ${err.message}`))
      debugLog.dump(err.token, false, true)
    }
    return
  }
...
module.exports = { printError, printParseError }
