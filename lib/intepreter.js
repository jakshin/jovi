...
/**
 * Executes the Rockstar program represented by the given AST root node.
 */
function execute(astRootNode) {
  // hoist functions, which must be declared as direct children of the root AST node
  astRootNode.children.filter((node) => node.op === "function").forEach((node) => {
    const functionName = node.var
    state.functions[functionName] = node
  })

  // run the program
  executeNodeChildren(astRootNode)
}
