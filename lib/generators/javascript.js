...
function generateExpression(expNode) {
  ...
  function generate(astNode) {
    ...
    if (astNode.op === "boolean") {
      ...
      else if (astNode.op === "math") {
        requireHelpers("_isnum", "_type")
      }
...
function generateVariableOrLiteral(astNode) {
  ...
  else if (astNode.literalType === "string") {
    let escaped = astNode.literalValue.replace(/\\/g, "\\\\")       // escape JS's escape char
    escaped = escaped.replace(/\${/g, "\\${").replace(/`/g, "\\`")  // escape template literal syntax
    return `\`${escaped}\``  // serialize as a template literal, for multiline string support
  }
  else {
...
const generators = {
  "assign": (node) => {
    ...
    if (node.expression) {
      ...
      const value = generateVariableOrLiteral(node)
      return `var ${variableName} = ${value};\n`
    }
  },

  "increment or decrement": (node) => {
    // { op: "increment or decrement", scale, var }
    // !!! scope? declared?
    requireHelpers("_incOrDec", "_type")
    const variableName = cleanVariableName(node.var)
    return `${variableName} = _incOrDec(${variableName}, "${variableName}", ${node.scale});\n`
  },
...
const helpers = {
  // increment/decrement
  _incOrDec: `function _incOrDec(val, varName, scale) {
    if (val === null) return scale

    const type = _type(val)
    if (type === "string" || type === "mysterious") {
      const operation = (scale > 0) ? "increment" : "decrement"
      throw new TypeError("Can't " + operation + " " + type + ' variable "' + varName + '"')
    }

    if (type === "boolean") {
      for (let i = 0; i < Math.abs(scale); i++) val = !val
      return val
    }
  ...
  _add: `function _add(left, right) {
    ...
    const leftType = _type(left), rightType = _type(right)
  ...
  _subtract: `function _subtract(left, right) {
    if (_isnum(left, right)) return left - right
    throw new TypeError("Can't subtract " + _type(right) + " from " + _type(left) + "; subtraction requires two numbers")
  }`,
  ...
  _divide: `function _divide(left, right) {
    ...
    if (_isnum(left, right)) return left / right
    throw new TypeError("Can't divide " + _type(left) + " by " + _type(right) + "; division requires two numbers")
  }`,
  ...
  _type: `function _type(val) {
    if (val === undefined) return "mysterious"
