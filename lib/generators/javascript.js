...
function generate(ast) {
  ...
  if (helpersRequired.has("_readInput")) {
    code = `(async function() {\n\n${code}\n})();\n`
  }
...
function generateExpression(expNode) {
  ...
  function generate(astNode) {
    ...
    if (astNode.op === "boolean") {
      ...
      if (astNode.op === "math") requireHelpers("_numericish")
      else if (astNode.operator === "==" || astNode.operator === "!=") requireHelpers("_strEq", "_numEq", "_boolEq")
...
const helpers = {
  ...
  _add: `function _add(left, right) {
    if (_numericish(left, right)) return left + right
  ...
  _multiply: `function _multiply(left, right) {
    ...
    const leftType = typeof left, rightType = typeof right
    if (leftType === "string" && _numericish(right)) return repeatStr(left, right)
    if (_numericish(left) && rightType === "string") return repeatStr(right, left)
  ...
  _numericish: `function _numericish(...vals) {
  ...
  _strEq: `function _strEq(str, other, otherType) {
    ...
    if (otherType === "number") {
      if (!/^[+-]?[\\d.]+$/.test(str) || str.replace(/[^.]/g, "").length > 1) return false
      const num = str.includes(".") ? parseFloat(str) : parseInt(str)
  ...
  _numEq: `function _numEq(num, other, otherType) {
    ...
    if (otherType === "string") return _strEq(other, num, "number")
    return num === other
  }`,
  ...
  _boolEq: `function _boolEq(bool, other, otherType) {
    ...
    if (otherType === "string") return _strEq(other, bool, "boolean")
    return bool === other
  }`,
}
