...
const helpers = {
  ...
  _subtract: `function _subtract(left, right) {
    ...
    throw new TypeError("Can't subtract " + rightType + " from " + leftType + "; subtraction requires two numbers")
  }`,
  ...
  _divide: `function _divide(left, right) {
    if (left === null) left = 0
    if (right === null) right = 0
    if (right === 0) throw new RangeError("Can't divide by zero")

    const leftType = typeof left, rightType = typeof right
    if (leftType === "number" && rightType === "number") return left / right
    throw new TypeError("Can't divide " + leftType + " by " + rightType + "; division requires two numbers")
  }`,
