  ...
  function generate(astNode) {
    ...
    if (astNode.op === "boolean") {
      ...
      if (astNode.op === "compare") {
        requireHelpers("_numeric")  // needed by _strEq(), which is needed by _eq(); and _less()
...
const helpers = {
  ...
  _less: `function _less(left, right) {
    ...
    if (leftType === "string" && rightType === "number" && _numeric(left))
      return _less(left.includes(".") ? parseFloat(left) : parseInt(left), right)
    if (rightType === "string" && leftType === "number" && _numeric(right))
      return _less(left, right.includes(".") ? parseFloat(right) : parseInt(right))
