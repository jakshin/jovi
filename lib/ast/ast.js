 ...
 * the AST's internal structures are still exposed directly to the interpreter and code generators.
 */
class AST {
  /**
  ...
  addAssignment(toVariableName, expression) {
    ...
    this._add({ op: "assign", expression: expression.toAST(), toVar: toVariableName })
  }
  ...
  addAssignmentWithLiteral(toVariableName, literalType, literalValue) {
    ...
    this._add({ op: "assign", expression: undefined, literalType, literalValue, toVar: toVariableName })
  }

  /**
   * Adds an increment or decrement statement to the current block.
   * Pass a positive integer in `scale` to increment, or a negative integer to decrement.
   */
  addIncrementOrDecrement(variableName, scale) {
    astChecker.checkName(variableName, "AST.addIncrementOrDecrement")
    astChecker.checkScale(scale, "AST.addIncrementOrDecrement")
    this._add({ op: "increment or decrement", scale, var: variableName })
  }
  ...
  addFunctionDeclaration(functionName, argumentList) {
    ...
    this._add({ op: "function", fn: functionName, args: argumentList.toAST() }, true)
  }
  ...
  addFunctionCall(functionName, argumentList) {
    ...
    this._add({ op: "call", fn: functionName, args: argumentList.toAST() })
  }
  ...
  addFunctionReturnWithLiteral(literalType, literalValue) {
    ...
    this._add({ op: "return", var: undefined, literalType, literalValue })
  }
  ...
  addInput(toVariableName) {
    ...
    this._add({ op: "input", toVar: toVariableName || undefined })
  }
   ...
   * using an expression (which can be as simple as a variable name).
   */
  addOutput(expression) {
    astChecker.checkExpression(expression, "AST.addOutput")
    this._add({ op: "output", expression: expression.toAST() })
  }
