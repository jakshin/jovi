...
const COMPARISON = ["is", "ain't", "aint"]  // TODO: should also allow "are", "aren't", "isn't"
...
const state = {
  ...
  justClosedIfBlock: false  // did we just complete an `if` or `else if` block?
}
...
function parseAllStatements(ast) {
  while (true) {
    ...
    if (token.type === TokenType.END_ALL_BLOCKS) {
      while (ast.currentOp) ast.endBlock()
      state.justClosedIfBlock = false  // can't if/else across concatenated files
      continue
    }
    if (token.type === TokenType.BLANK_LINE) {
      if (ast.currentOp) {
        state.justClosedIfBlock = (ast.currentOp === "if" || ast.currentOp === "else if")
        ast.endBlock()
      }
      continue
    }
    ...
    if (token.value === "put") {
      ...
      ast.add({
        op: (token.value === "build") ? "increment" : "decrement",
        var: variableName
      })
    }
      ...
      // conditional block
      // TODO: should allow "otherwise" as a synonym for "else"; "or if" for "else if"?

      if (token.value === "else" && !state.justClosedIfBlock) {
      ...
      ast.addBlock({
        op: keyword,
        expression: (keyword === "else") ? undefined : parseExpression()
      })
    }
      ...
      const keyword = startsMultiWordKeyword(token, "break it down") ? "break it down" : token.str
      if (!ast.inside("while")) {
    ...
    else if (token.value === "continue" || (token.value === "take" && startsMultiWordKeyword(token, "take it to the top"))) {
      // continue in loop
      ...
      if (startsMultiWordKeyword(token, "listen to")) {
      ...
      const { variableName, value, valueType } = parseVariableOrLiteral()

      if (!variableName && !valueType) {
      ...
      if (ASSIGMENT.includes(keyword)) {
        let value, valueType, expression

        if (keyword === "says") {
          // poetic string literal
          value = parsePoeticStringLiteral(popLine())
          valueType = "string"
        }
        else {
          ...
          if ([TokenType.NUMERIC_LITERAL, TokenType.STRING_LITERAL].includes(nextToken.type) || ALL_LITERALS.includes(nextToken.value)) {
            ...
            value = parsePoeticNumberLiteral(popLine())
            valueType = "number"
          }
        }
        ...
        const args = parseFunctionArguments(false)
        ast.addBlock({ op: "function", var: variableName, arguments: args })
      }
      else if (keyword === "taking") {
        // function call
        const args = parseFunctionArguments(true)
        ast.add({ op: "call", fn: variableName, arguments: args })
      }
      else {
    ...
    else if (token.value === "give" && startsMultiWordKeyword(token, "give back")) {
      // return from function
      if (!ast.inside("function")) {
        throw { message: `Unexpected keyword '${token.str}' here; must be used inside a function`, token }
      }

      let variableName, value, valueType
      ...
      if (!more() || peek().type === TokenType.BLANK_LINE) {
        ...
        if (nextToken.type === TokenType.NUMERIC_LITERAL) {
          // numeric literal
          nextToken = pop(false)
          calculateNumericValue(nextToken)
          ...
          // type literal
          nextToken = pop();
          ({ value, valueType } = parseTypeLiteral(nextToken))
        }
 ...
 * Returns an "expression" AST node which contains an array of AST nodes which comprise the expression's terms
 * ("boolean", "math", "compare", or "operand"). At least one "operand" will always be present, and first.
 */
function parseExpression() {
  ...
  while (true) {
    if (expectingVariableOrValue) {
      ...
      // we might have a arithmetic, comparison, or boolean operator next, or "taking" to start a function call,
      // or the expression might be complete
      const nextToken = peek()
      ...
      if (ALL_ARITHMETIC.includes(nextToken.value)) {
        ...
        const precedence = nextToken.value === "or" ? 1 : 2
 ...
 * Returns an array of "argument" AST nodes with either `var` or `value`/`valueType` properties.
 *
 * For function declarations, each argument must be a variable name;
 * for function calls, they can also be numeric/string/type literals.
 * In either case, arguments may be separated by commas and/or the "and" keyword.
 */
function parseFunctionArguments(forFunctionCall) {
  ...
  while (true) {
    const token = peek()
    let variableName, value, valueType, punctuation

    if (forFunctionCall) {
      ({ variableName, value, valueType, punctuation } = parseVariableOrLiteral())
    }
    else {
    ...
    if (!variableName && !valueType) {
      ...
      const expecting = forFunctionCall ? "a variable name or literal value" : "a parameter name, using variable naming rules"
      throw { message: `Unexpected word '${token.str}' here; must be ${expecting}`, token }
    }
 ...
 * Returns an object containing four properties: `variableName`, `value`, `valueType`, and `punctuation`.
 * Populates either `variableName`, or `value` and `valueType`, but noth both; if neither is populated,
...
function parseVariableOrLiteral() {
  ...
  if (token.type === TokenType.NUMERIC_LITERAL) {
    ...
    ({ value, valueType } = parseTypeLiteral(token))
    punctuation = token.punctuation
  }
  else if (startsVariable(token)) {
    rewind(token);
  ...
  return { variableName, value, valueType, punctuation }
}
 ...
 * Returns an object with `value` and `valueType` properties.
 */
...
/**
 * Calculates a numeric literal token's numeric value.
 * Mutates the passed token.
 */
function calculateNumericValue(token) {
  if (token.type === TokenType.NUMERIC_LITERAL) {
    token.value = token.str.includes(".") ? parseFloat(token.str) : parseInt(token.str)

    if (isNaN(token.value) || !isFinite(token.value) || !(/^\d+$/.test(token.str) || /^\d+\.\d+$/.test(token.str))) {
      throw { message: `Invalid numeric literal '${token.str}'`, token }
    }
  }
}
 ...
 */
function startsMultiWordKeyword(currentToken, keyword) {
