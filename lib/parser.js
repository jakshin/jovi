...
function parseAllStatements(ast) {
  ...
  while (true) {
    ...
    if (token.value === "put") {
      ...
      //!!! value -> literal, valueType -> literalType; switch order
      ...
      else ast.addOutputWithLiteral(valueType, value)
    }
      ...
      if (ASSIGMENT.includes(keyword)) {
        let value, valueType, expressionTerms
        ...
        else ast.addAssignmentWithLiteral(variableName, valueType, value)
      }
        ...
        ast.addFunctionDeclaration(variableName, args)
      }
      else if (keyword === "taking") {
        // function call
        const args = parseFunctionArguments(true)
        ast.addFunctionCall(variableName, args)
      }
      else {
      ...
      if (!more() || peek().type === TokenType.BLANK_LINE) {
        ...
        else ast.addFunctionReturnWithLiteral(valueType, value)
      }
    }
  ...
  while (true) {
    if (expectingVariableOrValue) {
      ...
      if (variableName && more() && peek().value === "taking") {
        ...
        pop()  // consume "taking"
        const args = parseFunctionArguments(true)
        terms.push({ type: "operand", fn: variableName, arguments: args })
      }
      else {
        terms.push({ type: "operand", var: variableName, value, valueType })
      }
 ...
 * Returns an array of objects containing `var`, `value` and `valueType` properties (some of which will be undefined).
 * The array might be empty.
 *
 * For function declarations, each argument must be a variable name, and only `var` will ever be set.
 * For function calls, arguments can also be numeric/string/type literals, and `valueType`/`value` might be set instead.
...
function parseFunctionArguments(forFunctionCall) {
  ...
  while (true) {
    ...
    args.push({ var: variableName, value, valueType })
 ...
 */
//!!! should just return the value instead of mutating (only default tokens should have `value`)
 ...
 */
//!!! should rename this to something better
