...
function executeCode(src) {
  const astRoot = parseToAST(src)

  // !!! catch runtime errors which are due to bad input and display them with context,
  // without displaying anything about unexpected internal errors
  debugLog.header("Executing")
  interpreter.execute(astRoot)
}
 ...
 *    If not given, each file is converted independently and the result is written alongside the input.
...
function convert(filePaths, targetLanguage, options = {}) {
  ...
  if (options.bundlePath) {
    const src = readRockstarFiles(filePaths, true)
    ...
    for (const fileNum in filePaths) {
      ...
      const src = readRockstarFile(filePath, true)
      const converted = convertCode(src, targetLanguage)
  ...
  writeTextFiles(convertedFiles, options.overwrite)
}
...
function convertCode(src, targetLanguage) {
  ...
  debugLog.header(`Converting to ${targetLanguage}`)
  const converted = generator.generate(astRoot)
 ...
 * @returns {Object} The AST's root node; you can pass it to Jovi's interpreter or one of its code generators.
 * @private
 */
function parseToAST(src) {
  try {
    ...
    if (err.token) {
      printParseError(err.message, err.token, src)
    }
