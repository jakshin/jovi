...
const helpers = {
  ...
  "-": (left, right) => {
    ...
    throw { message: `Can't subtract ${rightType} from ${leftType}; subtraction requires two numbers`, runtime: true }
  },
  "*": (left, right) => {
    if (left === null) left = 0
    if (right === null) right = 0
    ...
    if (leftType === "string" && rightType === "number") return repeatStr(left, right)
    if (leftType === "number" && rightType === "string") return repeatStr(right, left)
  ...
  "/": (left, right) => {
    if (left === null) left = 0
    if (right === null) right = 0
    if (right === 0) throw { message: "Can't divide by zero", runtime: true }

    const leftType = typeof left, rightType = typeof right
    if (leftType === "number" && rightType === "number") return left / right
    throw { message: `Can't divide ${leftType} by ${rightType}; division requires two numbers`, runtime: true }
  }
}
