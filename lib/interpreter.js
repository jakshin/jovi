...
const helpers = {
  ...
  "+": (left, right) => {
    if (numericish(left, right)) return left + right
  ...
  "*": (left, right) => {
    ...
    const leftType = typeof left, rightType = typeof right
    if (leftType === "string" && numericish(right)) return repeatStr(left, right)
    if (numericish(left) && rightType === "string") return repeatStr(right, left)
