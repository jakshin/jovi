...
const executors = {
  ...
  "increment or decrement": (node) => {
    // { op: "increment or decrement", scale, var: variableName }
    const scope = currentVariableScope()
    const variableType = typeOf(scope[node.var])

    if (variableType === "string" || variableType === "mysterious") {
      const operation = (node.scale > 0) ? "increment" : "decrement"
      throw { message: `Can't ${operation} ${variableType} variable "${node.var}"`, runtime: true }
    }

    if (variableType === "boolean") {
      for (let i = 0; i < Math.abs(node.scale); i++) scope[node.var] = !scope[node.var]
    }
    else if (variableType === "null") scope[node.var] = node.scale  // convert to 0, then increment/decrement
    else scope[node.var] += node.scale  // number (integer or float)
  },
...
/**
 * Gets a variable's type, like `typeof`,
 * but using "mysterious" for `undefined` and "null" for `null`.
 */
function typeOf(val) {
