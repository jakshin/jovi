...
const { isNumeric } = require("./utils/strings")
...
function evalExpression(expNode) {
  ...
  function evalExpressionNode(astNode) {
    ...
    if (astNode.op === "compare") {
      ...
      // !!! implement Rockstar comparison semantics... when done this block can just combine with the "math" block below
      const helperFn = helpers[astNode.operator]
      if (helperFn) return helperFn(leftVal, rightVal)
...
function isLess(left, right) {
  ...
  if (leftType === "string" && rightType === "number" && isNumeric(left))
    return isLess(left.includes(".") ? parseFloat(left) : parseInt(left), right)
  if (rightType === "string" && leftType === "number" && isNumeric(right))
    return isLess(left, right.includes(".") ? parseFloat(right) : parseInt(right))
